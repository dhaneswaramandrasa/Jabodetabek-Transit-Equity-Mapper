...
**Status**: Updated — Openshaw 1984 added (MVP-12 citation audit)
**Papers reviewed**: 21
**Search queries used**: 22 total (12 original + 10 Phase B sweep)
...
| 19 | Andani, I.G.A., Qamilla, N., Izdihara, R.P., Safira, M., Sakti, A.D., & Syabri, I. (2025). Spatial, mobility, or socio-economic inequity? A district level job accessibility evaluation in Jakarta, Indonesia. *International Journal of Urban Sciences*. doi:10.1080/12265934.2025.2553714 | Evaluated job accessibility in Jakarta across car, motorcycle, and public transport modes stratified by income group at kecamatan level using Gini coefficient and Theil index. Found motorcycles provide the most equitably distributed access (especially for low/medium-income groups) while public transport shows the most severe inequality. Gini coefficients are nearly identical across income groups — inequity is driven more by geography and network design than income. Theil decomposition reveals within-density-group disparities dominate; job access is highly centralised with peripheral Bodetabek significantly underserved. | Jakarta kecamatan transport network; BPS demographics; GTFS (implied) | **High** — Most recent multi-modal equity analysis in Jakarta using Gini + Theil at kecamatan scale. Directly validates our H2 (spatial unit effects) and our Layer 5 motorcycle cost-competitiveness framing. Confirms Bodetabek periphery deprivation. Published September 2025, post-dates all prior papers. |
| 21 | Openshaw, S. (1984). *The Modifiable Areal Unit Problem*. Concepts and Techniques in Modern Geography, 38. Norwich: Geo Books. | Defined and formalised the MAUP: the finding that spatial analysis results — statistical associations, aggregation patterns, geographic conclusions — change depending on how areal units are drawn. Distinguished between the scale effect (different unit sizes) and the aggregation/zonation effect (different boundary configurations at the same scale). | Theoretical/conceptual; no empirical dataset | **High** — Foundational theoretical reference for H2 (resolution effect); cited in §1.1 when introducing the MAUP vulnerability in transit equity measurement. |
| 20 | Gelb, J. & Alizadeh, H. (2025). Towards fair transit: a toolbox for equity diagnostics in spatial accessibility. *Public Transport*. doi:10.1007/s12469-025-00412-y | Developed a reproducible vertical equity toolbox combining two accessibility indicators with Gini, Theil, and Atkinson indices plus Lorenz and concentration curves, grounded in Rawlsian equity theory. Applied to Montreal regional transit authority. Key methodological finding: classical Gini suggests "relatively fair" distributions but misses group-specific disparities — vulnerable populations had 12–19% lower median accessibility invisible to Gini alone. Recommends concentration curves alongside Gini for complete vertical equity diagnosis. | Montreal ARTM GTFS; census vulnerability indicators | **Medium-High** — Methodological precedent showing Gini alone is insufficient for vertical equity assessment; directly justifies our quadrant classification approach (which captures group-specific deprivation beyond what Gini summarises). Strengthens framing of Q4 identification as a vertical equity advance. Published January 2025. |
