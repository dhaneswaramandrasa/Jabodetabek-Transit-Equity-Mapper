...
export default function PlannerLens() {
  const whatIfOpen = useAccessibilityStore((s) => s.whatIfOpen);
  const setWhatIfOpen = useAccessibilityStore((s) => s.setWhatIfOpen);
  const whatIfScenarios = useAccessibilityStore((s) => s.whatIfScenarios);
  const removeWhatIfScenario = useAccessibilityStore((s) => s.removeWhatIfScenario);
          ...
          <div className="glass-panel rounded-xl p-5">
            ...
            <div className="space-y-2">
              {whatIfScenarios.map((s) => (
                <div
                  key={s.id}
                  className="flex items-center justify-between bg-surface-container rounded px-3 py-2"
                >
                  <div>
                    <p className="text-xs font-label text-on-surface font-semibold">
                      {s.label}
                    </p>
                    <p className="text-[10px] font-mono text-on-surface-variant">
                      {s.params.mode} · {s.params.headwayMin}min
                    </p>
                  </div>
                  <button
                    onClick={() => removeWhatIfScenario(s.id)}
                    className="text-on-surface/30 hover:text-error transition-colors text-sm font-mono"
                  >
                    ✕
                  </button>
                </div>
